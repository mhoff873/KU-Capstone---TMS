...
def getAllCompletedSteps(uname, taskid):
    ...
    cur.execute('''SELECT mainSteps.title, completedSteps.dateTimeCompleted, completedSteps.timeSpent
                ...
                WHERE completedSteps.mainStepID = mainSteps.mainStepID AND completedTasks.completedTaskID=completedSteps.completedTaskID AND completedTasks.userID = users.userID AND mainSteps.taskID = %d AND users.email = "%s" AND completedSteps.dateTimeCompleted IS NOT NULL''' % (
    int(taskid), uname,))
    r = cur.fetchall()
    count = 1
    lst = []
...
def getAllCompletedTasksByUser(uname):
    ...
    WHERE completedTasks.userID=%d AND completedTasks.taskID=task.taskID AND dateTimeCompleted IS NOT NULL''' % (
    int(user),))
    r = cur.fetchall()
    count = 1
    lst = []
    for s in r:
        ...
        spc = str(s["dateTimeCompleted"]).find(' ')
        ufc = date_string[:spc] + "T" + date_string[spc + 1:]
        d = {"$id": str(count), "taskID": int(s['taskID']), "taskName": str(s['title']), "assignedUser": str(uname),
             "dateTimeCompleted": ufc, "totalTime": int(s['totalTime']),
             "totalDetailedStepsUsed": int(s['detailedStepsUsed']), "id": int(count)}
        lst.append(d)
        count += 1
    return lst
...
def postSurveyResults(SR, SQR):
    ...
    int(SR['userID']), int(SR['formID']), str(SR['name']), int(SR['timeSpent']), str(SR['email']), str(SR['ipAddr']),
    int(SR['ageGroup']), str(SR['results']), str(SR['date']), str(SR['comments']),))
    # execute the query
    mysql.connection.commit()
...
def postSurveyForm(SF, SQ):
    ...
    cur.execute('''INSERT INTO surveyForm (`supervisorID`,`formTitle`,`description`,`dateCreated`,`dateModified`,`isActive`) 
            values(%d,"%s","%s","%s","%s",%d)''' % (
    int(SF['supervisorID']), str(SF['formTitle']), str(SF['description']), str(SF['dateCreated']),
    str(SF['dateModified']), int(SF['isActive']),))
    mysql.connection.commit()
    # get the auto increment value
    cur.execute('SELECT LAST_INSERT_ID()')
    id = cur.fetchone()
    ...
    for r in SQ:
        ...
        cur.execute('''INSERT INTO surveyQuest (`formID`,`questType`,`questText`,`isActive`,`questOrder`)
                values(%d,"%s","%s",%d,%d)''' % (
        int(id['LAST_INSERT_ID()']), str(r['questType']), str(r['questText']), int(r['isActive']),
        int(r['questOrder']),))
        mysql.connection.commit()
...
def getCompletedTasksByID(date, ID):
    ...
    # query the database to get the data on completed tasks that occur after the provided date
    cur.execute(
        'SELECT task.taskID, users.userID, users.fname, users.lname, completedTasks.completedTaskID, task.title, completedTasks.totalTime, completedTasks.dateStarted, completedTasks.dateTimeCompleted, completedTasks.detailedStepsUsed, completedTasks.ipAddr FROM users, task, completedTasks WHERE users.userID=completedTasks.userID AND completedTasks.taskId=task.taskID AND completedTasks.dateTimeCompleted>="%s" AND completedTasks.taskID = %d' % (
        str(date), int(ID),))
    taskData = cur.fetchall()
    lstTask = []
    # iterate over the data to get the completed steps
    for t in taskData:
        taskDict = {'taskID': t['taskID'], 'userID': t['userID'], 'fname': t['fname'], 'lname': t['lname'],
                    'title': t['title'], 'totalTime': t['totalTime'], 'dateStarted': t['dateStarted'],
                    'dateTimeCompleted': t['dateTimeCompleted'], 'detailedStepsUsed': t['detailedStepsUsed'],
                    'ipAddr': t['ipAddr']}
        taskDict['detailedSteps'] = getCompletedStepsByID(t['completedTaskID'])
        lstTask.append(taskDict)
    # sort by date time completed
...
def getCompletedStepsByID(ID):
    ...
    cur.execute(
        'SELECT mainSteps.title, mainSteps.listOrder, completedSteps.detailedStepsUsed, completedSteps.timeSpent, completedSteps.dateTimeCompleted FROM mainSteps, completedSteps WHERE completedSteps.mainStepID=mainSteps.mainStepID AND completedSteps.completedTaskID = %d' % (
        int(ID),))
...
def getTasksCreatedByID(superID):
    ...
    # query the database to get the data on the completed steps
    cur.execute(
        'SELECT task.title, task.taskID FROM task WHERE task.published=1 AND task.activated=1 AND task.supervisorID = %d' % (
        int(superID),))
    tasks = cur.fetchall()
    lstTasks = []
...
def getUncompletedTaskByID(userID, taskID):
    ...
    # query the database to get the data on the completed steps
    cur.execute(
        'SELECT count(completedTasks.taskID) FROM completedTasks WHERE completedTasks.dateTimeCompleted IS NULL AND completedTasks.userID = %d AND completedTasks.taskID=%d' % (
